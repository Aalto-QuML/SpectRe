...
void compute_spectre_raw(
    ...
    auto process_component = [&](int_t root,
        ...
        // Power Method to compute the largest eigenvalue
            torch::Tensor x = torch::randn({(int_t)n}, torch::kFloat32); // Random initial vector
            x = x / x.norm(); // Normalize the vector
            torch::Tensor eigenvalue = torch::tensor(0.0, torch::kFloat32); // Initialize eigenvalue

            const int max_iterations = 1000; // Maximum number of iterations
            const float tolerance = 1e-4;    // Convergence tolerance

            for (int iter = 0; iter < max_iterations; ++iter) {
                torch::Tensor next_x = torch::matmul(L, x); // Multiply L by the current vector
                next_x = next_x / next_x.norm();           // Normalize the resulting vector

                // Compute the Rayleigh quotient as the eigenvalue approximation
                torch::Tensor next_eigenvalue = torch::dot(next_x, torch::matmul(L, next_x));

                // Check for convergence
                if (torch::abs(next_eigenvalue - eigenvalue).item<float>() < tolerance) {
                    eigenvalue = next_eigenvalue;
                    break;
                }

                x = next_x;             // Update the vector
                eigenvalue = next_eigenvalue; // Update the eigenvalue
            }

            eigvals= torch::tensor({eigenvalue.item<float>()}, torch::kFloat32);
        // Store the largest eigenvalue in a tensor
        }
        else{
            auto [eigenvals, eigvecs] = torch::linalg_eigh(L);
            eigvals= eigenvals;
        }
        ...
        try {
            ...
            if (younger == older) {
                ...
                if ((i == n_edges - 1) || filtered_e[cur_edge_index] != filtered_e[sorting_space[edge_begin + i + 1]]) {
                    if (i % 3 == 0) { // Schedule: process only half of the time
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("compute_spectre_scheduling_batched_mt",
